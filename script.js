function createElements(dataClient) {
   const vehicleRegistration = document.querySelector('.client-registration');
  ...
  clone.querySelector('.client-registration__email-client').textContent = dataClient.price;
  clone.querySelector('.client-registration__status').textContent = `Condition: ${dataClient.condition}`;
  clone.querySelector('.client-registration__number-phone-client').textContent = `Year: ${dataClient.year}`;
  ...
  buttonDelete.addEventListener('click',function criaFuncao(id) {
    return deleteCar(dataClient.id);
})

  const buttonEdit = clone.querySelector('.client-registration__button--edit')
  buttonEdit.addEventListener('click',function criaFuncao(id) {
  modal.style.display = "flex";
  document.getElementById('name').value = dataClient.name,
  document.getElementById("price").value = dataClient.price,
  document.getElementById("year").value = `${dataClient.year}`
  document.getElementById("km").value = `${dataClient.km}`;
  document.getElementById("condition").value = `${dataClient.condition}`;
  document.getElementById("img").value = dataClient.imgSrc
...
const btnMoreInfo = document.querySelectorAll('.client-registration__button--more-info');
